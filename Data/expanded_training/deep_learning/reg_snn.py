"""G18: SNN - Self-Normalizing Neural Networks

With Optuna hyperparameter tuning.

Architecture: Dense layers with SELU activations and AlphaDropout.
...
def build_snn(input_shape,
    ...
    Args:
        ...
        hidden_units: List of units for hidden layers
        dropout_rate: AlphaDropout rate (lower than regular dropout)
    ...
    for units in hidden_units:
        ...
        x = layers.AlphaDropout(dropout_rate)(x)  # Preserves self-normalizing property
...
def build_deep_snn(input_shape,
                   hidden_units=[1024, 512, 256, 128, 64],
                   dropout_rate=0.1,
                   l2_reg=1e-5):
    """
    Deeper SNN variant for complex patterns.

    Uses more layers with gradual dimension reduction.
    Self-normalizing property allows training deeper networks.
    """
    inputs = layers.Input(shape=(input_shape,))
    ...
    # Hidden layers with residual-like connections every 2 layers
    for i, units in enumerate(hidden_units[1:], 1):
        ...
        h = layers.Activation('selu')(h)
        h = layers.AlphaDropout(dropout_rate)(h)

        # Add residual connection if dimensions match
        if i % 2 == 0 and x.shape[-1] == units:
            x = layers.Add()([x, h])
        else:
            x = h
    ...
    model = Model(inputs, outputs, name='Deep_SNN')
    return model


def build_snn_with_skip(input_shape,
                        block_units=[512, 256, 128],
                        dropout_rate=0.1,
                        l2_reg=1e-5):
    """
    SNN with skip connections for improved gradient flow.

    Combines self-normalizing properties with residual learning.
    """
    inputs = layers.Input(shape=(input_shape,))
    ...
    for i, units in enumerate(block_units):
        ...
        if skip.shape[-1] != units:
            skip = layers.Dense(units, kernel_initializer='lecun_normal')(skip)
    ...
    # Output
    x = layers.Dense(32, kernel_initializer='lecun_normal')(x)
    x = layers.Activation('selu')(x)
    outputs = layers.Dense(1, kernel_initializer='lecun_normal')(x)

    model = Model(inputs, outputs, name='SNN_Skip')
    return model
...
def plot_results(history, metrics, output_dir='.'):
    ...
    plt.title('SNN: Training and Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(output_dir, 'snn_loss_curve.png'), dpi=150)
    plt.close()
    ...
    plt.title(f"SNN: Actual vs Predicted (R²={metrics['r2']:.4f})")
    plt.xlabel('Actual')
    plt.ylabel('Predicted')
    plt.savefig(os.path.join(output_dir, 'snn_predictions.png'), dpi=150)
    plt.close()
    ...
    plt.title('SNN: Residuals Distribution')
    plt.xlabel('Residual')
    plt.ylabel('Frequency')
    plt.savefig(os.path.join(output_dir, 'snn_residuals.png'), dpi=150)
    plt.close()
...
def main(train_path: str, test_path: str, weights_path: str = None,
    ...
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    setup_logging_for_progress(os.path.join(OUTPUT_DIR, 'training_snn.log'))
    check_gpu()
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Train SNN with Optuna tuning')
