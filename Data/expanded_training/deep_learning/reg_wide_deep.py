"""Wide & Deep Network for Tabular Data Regression

With Optuna hyperparameter tuning.

Architecture: Combines a wide (linear) component for memorization with
a deep component for generalization.
"""
...
def build_wide_deep(input_shape,
    ...
    Args:
        ...
        deep_units: List of units for deep component
        dropout_rate: Dropout rate for deep component
        l2_reg: L2 regularization for deep component
        wide_l1_reg: L1 regularization for wide component (sparsity)
    ...
    # === WIDE COMPONENT ===
    # Linear model with L1 regularization for sparse feature selection
    wide = layers.Dense(
        1,
    ...
    # === COMBINE WIDE + DEEP ===
    # Weighted combination of wide and deep outputs
...
def build_wide_deep_v2(input_shape,
                       deep_units=[512, 256, 128, 64],
                       dropout_rate=0.3,
                       l2_reg=1e-4):
    """
    Alternative Wide & Deep with concatenation before final layer.

    This version concatenates wide and deep representations before
    a final dense layer, allowing learned combination weights.
    """
    inputs = layers.Input(shape=(input_shape,))

    # === WIDE COMPONENT ===
    # Compressed wide representation
    wide = layers.Dense(
        32,
        activation='relu',
        kernel_regularizer=keras.regularizers.l1_l2(l1=1e-5, l2=l2_reg),
        name='wide_compress'
    )(inputs)
    ...
    combined = layers.Dropout(dropout_rate / 2)(combined)
    outputs = layers.Dense(1)(combined)

    model = Model(inputs, outputs, name='Wide_Deep_V2')
    return model
...
def plot_results(history, metrics, output_dir='.'):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(output_dir, 'wide_deep_loss_curve.png'), dpi=150)
    plt.close()
    ...
    plt.title(f"Wide & Deep: Actual vs Predicted (R²={metrics['r2']:.4f})")
    plt.xlabel('Actual')
    plt.ylabel('Predicted')
    plt.savefig(os.path.join(output_dir, 'wide_deep_predictions.png'), dpi=150)
    plt.close()
    ...
    plt.title('Wide & Deep: Residuals Distribution')
    plt.xlabel('Residual')
    plt.ylabel('Frequency')
    plt.savefig(os.path.join(output_dir, 'wide_deep_residuals.png'), dpi=150)
    plt.close()
...
def main(train_path: str, test_path: str, weights_path: str = None,
    ...
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    setup_logging_for_progress(os.path.join(OUTPUT_DIR, 'training_wide_deep.log'))
    check_gpu()
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Train Wide & Deep with Optuna tuning')
