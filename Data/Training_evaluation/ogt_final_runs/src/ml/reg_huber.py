...
log_file_name = f"huber_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("Huber training script started")
...
# MODEL NAME
MODEL_NAME = "huber_rmse.pkl"
MODEL_FOLDER = "huber_rmse"
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        model = HuberRegressor(**best_params, max_iter = 10000)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", HuberRegressor(**best_params, max_iter=10000)),
            ]
        )
