...
log_file_name = f"mlp_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("MLP training script started")
...
# MODEL NAME
MODEL_NAME = "mlp_rmse.pkl"
MODEL_FOLDER = "mlp_rmse"
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups=groups):
        ...
        model = MLPRegressor(**best_params, max_iter=10000, random_state = RANDOM_STATE)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", MLPRegressor(**best_params, random_state = RANDOM_STATE)),
            ]
        )
