"""
REGRESSION WITH BAYESIAN RIDGE MODEL
...
log_file_name = f"bayesian_ridge_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("Bayesian Ridge training script started")
...
# MODEL NAME
MODEL_NAME = "bayesian_ridge_rmse.pkl"
MODEL_FOLDER = "bayesian_ridge_rmse"
...
def objective(trial, X, y, groups):
    param = {
        "alpha_1": trial.suggest_float("alpha_1", 1e-7, 1e2, log=True),
        "alpha_2": trial.suggest_float("alpha_2", 1e-7, 1e2, log=True),
        "lambda_1": trial.suggest_float("lambda_1", 1e-7, 1e2, log=True),
        "lambda_2": trial.suggest_float("lambda_2", 1e-7, 1e2, log=True),
    }
