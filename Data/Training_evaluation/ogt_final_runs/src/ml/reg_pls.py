...
log_file_name = f"pls_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("PLS training script started")
...
# MODEL NAME
MODEL_NAME = "pls_rmse.pkl"
MODEL_FOLDER = "pls_rmse"
...
def objective(trial, X, y, groups):
    param = {
        "n_components": trial.suggest_int("n_components", 1, X.shape[1] - 1),
        "max_iter": 10000,
        "scale":False,
    }
    model = PLSRegression(**param)
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        model = PLSRegression(**best_params, max_iter=10000, scale=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", PLSRegression(**best_params, scale= False, max_iter=10000)),
            ]
        )
        ...
        for dataset_name, X, y in [
            ...
            if dataset_name == "Cross-Validation":
                ...
                # write y and ypred to file, so that I can later look at stdev:
