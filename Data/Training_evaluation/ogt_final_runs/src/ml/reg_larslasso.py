"""
REGRESSION WITH LARSLASSO MODEL
...
log_file_name = f"larslasso_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("LassoLarsCV training script started")
...
# MODEL NAME
MODEL_NAME = "larslasso_rmse.pkl"
MODEL_FOLDER = "larslasso_rmse"
...
def objective(trial, X, y, groups):
    ...
    def custom_scorer(estimator, X, y):
        ...
        return -extreme_rmse(y, y_pred, X.shape[1])
    """
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        model = LassoLars(**best_params, fit_intercept = True, random_state = RANDOM_STATE, max_iter=10000)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", LassoLars(**best_params, random_state = RANDOM_STATE, fit_intercept=True, max_iter = 10000)),
            ]
        )
