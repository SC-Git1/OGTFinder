...
log_file_name = f"svr_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("SVR training script started")
...
# MODEL NAME
MODEL_NAME = "svr_rmse.pkl"
MODEL_FOLDER = "svr_rmse"
...
def objective(trial, X, y, groups):
    kernel = RBF(trial.suggest_float('length_scale', 1e-2, 1e2, log=True)) + WhiteKernel(noise_level=trial.suggest_float('noise_level', 0.1, 1e3, log=True))

    param = {
        "kernel": kernel,
        "C": trial.suggest_float("C", 1e-3, 1e2, log=True),
        'epsilon': trial.suggest_float('epsilon', 1e-3, 1e2, log=True),
    }
    model = SVR(**param)
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        model = SVR(kernel=kernel, C = best_params["C"], epsilon = best_params["epsilon"])
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", SVR(kernel = kernel, C = best_params["C"], epsilon = best_params['epsilon'])),
            ]
        )
