"""
REGRESSION WITH KERNEL RIDGE REGRESSION MODEL
...
log_file_name = f"kernel_ridge_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("Kernel Ridge Regression training script started")
...
# MODEL NAME
MODEL_NAME = "kernel_ridge_rmse.pkl"
MODEL_FOLDER = "kernel_ridge_rmse"
...
def objective(trial, X, y, groups):
    ...
    kernel = RBF(trial.suggest_float('length_scale', 1e-2, 1e2, log=True)) + \
               WhiteKernel(noise_level=trial.suggest_float('noise_level', 1e-2, 1e2, log=True))
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups=groups):
        ...
        kernel = RBF(best_params['length_scale']) + WhiteKernel(noise_level=best_params['noise_level'], noise_level_bounds = "fixed")
