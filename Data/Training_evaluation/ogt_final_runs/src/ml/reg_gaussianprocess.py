"""
REGRESSION WITH GAUSSIAN PROCESS MODEL
...
log_file_name = f"gaussian_process_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("Gaussian Process training script started")
...
# MODEL NAME
MODEL_NAME = "gaussian_process_rmse.pkl"
MODEL_FOLDER = "gaussian_process_rmse"
...
def objective(trial, X, y, groups):
    kernel = RBF(trial.suggest_float('length_scale', 1e-5, 1e5, log=True)) + \
             WhiteKernel(noise_level=trial.suggest_float('noise_level', 0.1, 1e6, log=True))
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        kernel = RBF(best_params['length_scale']) + WhiteKernel(noise_level=best_params['noise_level'])

        model = GaussianProcessRegressor(kernel = kernel, optimizer = None, random_state = RANDOM_STATE)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", GaussianProcessRegressor(kernel=kernel, random_state=RANDOM_STATE, optimizer=None)),
            ]
        )
        # alpha=best_params['alpha']
        ...
        for dataset_name, X, y in [
            ("Training", X_train_unscaled, y_train),
