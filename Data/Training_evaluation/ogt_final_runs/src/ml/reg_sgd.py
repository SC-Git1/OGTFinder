...
log_file_name = f"sgd_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("Sgd training script started")
...
# MODEL NAME
MODEL_NAME = "sgd_rmse.pkl"
MODEL_FOLDER = "sgd_rmse"
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        model = SGDRegressor(**best_params, max_iter = 10000, random_state = RANDOM_STATE)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", SGDRegressor(**best_params, max_iter = 10000, random_state = RANDOM_STATE)),
            ]
        )
