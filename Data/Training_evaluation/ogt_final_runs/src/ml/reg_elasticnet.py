...
log_file_name = f"elastic_net_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("Elastic Net training script started")
...
# MODEL NAME
MODEL_NAME = "elastic_net_rmse.pkl"
MODEL_FOLDER = "elastic_net_rmse"
...
def cross_validate_with_optimal_params(X, y, best_params, groups, cv=N_CV):
    ...
    for train_index, val_index in kf.split(X, groups = groups):
        ...
        model = ElasticNet(**best_params, max_iter = 50000, random_state = RANDOM_STATE)
...
if __name__ == "__main__":
    ...
    try:
        ...
        final_model = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("regressor", ElasticNet(**best_params, max_iter = 50000)),
            ]
        )
