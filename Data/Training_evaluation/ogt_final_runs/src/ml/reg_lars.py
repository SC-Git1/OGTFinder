...
log_file_name = f"lars_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.info("LARS training script started")
...
# MODEL NAME
MODEL_NAME = "lars_rmse.pkl"
MODEL_FOLDER = "lars_rmse"
...
def objective(trial, X, y, groups):
    param = {
        "n_nonzero_coefs": trial.suggest_int("n_nonzero_coefs", 1,  X.shape[1] - 1),
    }
    ...
    try:
        ...
        new_hyperparameters = {
            f"{EXPERIMENT_NAME_DEFAULT}": {
                f"{MODEL_FOLDER}": {
                    "n_nonzero_coefs": best_params["n_nonzero_coefs"],
                }
            }
        }
